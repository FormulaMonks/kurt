...
import { KurtResultLimitError, KurtTools } from "@formula-monks/kurt"
...
describe("KurtVertexAI generateWithOptionalTools", () => {
  test("calculator (with tool call)", async () => {
    const result = await snapshotAndMock((kurt) =>
      kurt.generateWithOptionalTools({
        prompt:
          "What's 9876356 divided by 30487, rounded to the nearest integer? Use the tool to calculate",
  ...
  */
  ...
  test("uses a kurt tool to search the web", async () => {
    ...
    expect(result.text).toEqual(
      "It is Friday, and the weather in Lisbon, Portugal is sunny with a temperature of 68°F (20°C) and a 0% chance of rain. \n"
    )
  })
  ...
  test("can use both a kurt tool and an external tool", async () => {
    const result = await snapshotAndMock((kurt) =>
      kurt.generateWithOptionalTools({
        prompt: [
          "Get the current weather in Lisbon and divide the temperature in Celsius by 0.79 using the tool",
        ].join("\n"),
        tools: {
          webSearch: KurtTools.WebSearch(),
          ...calculatorTools,
